#!/usr/bin/env bash
# session-health-check.sh — Phase 2: Session 健康检测 (FIXED v2)
# 作者：贝塔 🔵
# 创建：2026-03-18
# 修复：改用 jsonl 文件内时间戳检测活跃度，而非文件 mtime
# 用法：./scripts/session-health-check.sh [--notify] [--json]
...
check_session_health() {
  ...
  for f in "$sessions_dir"/*.jsonl; do
    ...
    # Get last timestamp from inside the jsonl file
    local result
    result=$(python3 -c "
import json, sys
last_ts = None
...
if last_ts:
    ...
    try:
        ...
        print(f'{int(dt.timestamp())} {last_ts}')
    except:
        pass
" 2>/dev/null)
